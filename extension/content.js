(() => {
  ...
  async function executeCall(call) {
    ...
    if (!ALLOWED_TOOLS.has(call.tool)) {
      console.warn("NOVUM: rejected unknown tool", call.tool);
      return;
    }
  ...
  function explicitRole(node) {
    ...
    const assistant = node.closest('[data-message-author-role="assistant"], [data-turn="assistant"]');
    if (assistant) return "assistant";
    const user = node.closest('[data-message-author-role="user"], [data-turn="user"]');
    if (user) return "user";
    return null;
  }
  ...
  function classifyTurn(node) {
    ...
    const labels = Array.from(
      turn.querySelectorAll('h1,h2,h3,h4,h5,h6,[class*="sr-only"],[aria-label]')
    )
      .slice(0, 30)
    ...
    if (/\b(chatgpt said|chatgpt a dit|assistant said)\b/i.test(labels) || /(^|\s)chatgpt($|\s)/i.test(labels)) {
      return "assistant";
    }
  ...
  // ChatGPT changes its turn DOM frequently. The interval is a cheap fallback for
  // cases where streaming mutations happen before role metadata is attached.
